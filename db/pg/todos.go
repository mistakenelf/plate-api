...
func DeleteTodoList(todoList *models.TodoList) {
	if err := gorm.DBCon().Where("todo_list_id = ?", todoList.ID).Delete(&models.Todo{}).Error; err != nil {
		panic(err)
	}

	if err := gorm.DBCon().Delete(&todoList).Error; err != nil {
		panic(err)
	}
}
...
func UpdateTodoList(todoList *models.TodoList) {
	if err := gorm.DBCon().Save(&todoList).Error; err != nil {
		panic(err)
	}
}
...
func DeleteTodo(todo *models.Todo) {
	if err := gorm.DBCon().Delete(&todo).Error; err != nil {
		panic(err)
	}
}
...
func UpdateTodo(todo *models.Todo) {
	if err := gorm.DBCon().Save(&todo).Error; err != nil {
		panic(err)
	}
}
